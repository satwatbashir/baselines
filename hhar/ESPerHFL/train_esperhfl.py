...
def main() -> None:
    ...
    work_w = HARNet(in_ch=trainset.X.shape[1], num_classes=NUM_CLASSES).to(device)
    work_v = HARNet(in_ch=trainset.X.shape[1], num_classes=NUM_CLASSES).to(device)
    served_model = HARNet(in_ch=trainset.X.shape[1], num_classes=NUM_CLASSES).to(device)  # for evaluation only
