...
def load_and_split(args):
    ...
    X, y = load_hhar_data(
        data_root=args.data_root, use_watches=True,
        sample_rate_hz=50, window_seconds=2, window_stride_seconds=1,
...
def main() -> None:
    ...
    # ---- Data + partition --------------------------------------------------
    print(f"[chpfl] device={device}  edges={args.num_edges}  clients/edge={args.clients_per_edge}  out={out_dir}")
    ...
    # ---- Models + state ----------------------------------------------------
    avg_model = HARNet(in_ch=trainset.X.shape[1], num_classes=NUM_CLASSES).to(device)
    ...
    work_model = HARNet(in_ch=trainset.X.shape[1], num_classes=NUM_CLASSES).to(device)
    cluster_eval_model = HARNet(in_ch=trainset.X.shape[1], num_classes=NUM_CLASSES).to(device)
