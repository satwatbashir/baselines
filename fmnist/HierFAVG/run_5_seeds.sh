#!/usr/bin/env bash
# Run HierFAVG on Fashion-MNIST for 5 seeds. Each seed writes a self-contained
# folder under ../gc_results/hierfavg_seed{seed}/.
set -euo pipefail
...
for SEED in $SEEDS; do
    OUT="$GC_RESULTS_DIR/hierfavg_seed${SEED}"
    LOG="$GC_RESULTS_DIR/hierfavg_seed${SEED}.log"
    echo "============================================================"
    echo "[run_5_seeds] seed=$SEED  out=$OUT"
    echo "============================================================"
    START=$(date +%s)
    "$PYTHON" -u train_hierfavg.py --seed "$SEED" --out-dir "$OUT" "$@" 2>&1 | tee "$LOG"
    END=$(date +%s)
...
echo "  (cd $(dirname "$GC_RESULTS_DIR") && zip -r hierfavg_fmnist_5seeds.zip gc_results)"
