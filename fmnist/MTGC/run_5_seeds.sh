#!/usr/bin/env bash
# Run MTGC on Fashion-MNIST for 5 seeds. Each seed writes a self-contained folder
...
for SEED in $SEEDS; do
    ...
    echo "============================================================"
    START=$(date +%s)
    "$PYTHON" -u train_mtgc.py --seed "$SEED" --out-dir "$OUT" "$@" 2>&1 | tee "$LOG"
    END=$(date +%s)
...
echo "  (cd $(dirname "$GC_RESULTS_DIR") && zip -r mtgc_fmnist_5seeds.zip gc_results)"
