#!/usr/bin/env bash
# Run CHPFL on Fashion-MNIST for 5 seeds. Each seed writes a self-contained
...
for SEED in $SEEDS; do
    ...
    echo "============================================================"
    START=$(date +%s)
    "$PYTHON" -u train_chpfl.py --seed "$SEED" --out-dir "$OUT" "$@" 2>&1 | tee "$LOG"
    END=$(date +%s)
...
echo "  (cd $(dirname "$GC_RESULTS_DIR") && zip -r chpfl_fmnist_5seeds.zip gc_results)"
